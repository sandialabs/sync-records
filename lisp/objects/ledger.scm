    ...
    '(define-class (ledger)
       ...
       (define (~signature-sign! self chain)
         (let* ((config ((self '~field) 'config))
           ...
           ((standard 'deep-set!) chain '(-1 (*crypto*)) '(nothing ()))
           ((standard 'deep-call!) chain '(-1)
            (lambda (record)
              ((record 'set!) '(*cypto* signature) `(content ,(crypto-sign secret-key (sync-digest (chain)))))
              ((record 'set!) '(*cypto* public-key) `(content ,public-key))))))

       (define (~signature-verify self chain public-key)
         (let* ((config ((self '~field) 'config))
                (standard ((self '~field) 'standard))
                (chain-copy ((standard 'load) (chain))))
           ((standard 'deep-set!) chain-copy '(1 (*meta* signature)) (sync-null))
           ((standard 'deep-call!) chain '(-1)
            (lambda (record)
              (let ((signature (cadr ((record 'get) '(*crypto* signature))))
                    (included-key (cadr ((record 'get) '(*crypto* public-key)))))
       ...
       (define (step-peer! self name)
         ;; todo: what if peer is deleted before?
         (let* ((config ((self '~field) 'config))
                (standard ((self '~field) 'standard))
                (perm ((self '~field) 'perm))
                (stage ((self '~field) 'stage))
                (last ((standard 'deep-get) perm `(-1 (*peer* ,name))))
                (index (if (not (eq? (car last) 'content)) -1 (- (((standard 'load) last) 'size) 1)))
           ...
           (if (= ((latest 'size)) 0)
               ...
               (begin ((self '~signature-verify) latest ((config 'get) `(private peer ,name public-key)))
                      ((stage 'set!) `(*peer* ,name valid?)
                       `(content ,(equal? ((last 'digest)) ((latest 'digest) (last 'index)))))))
           ((stage 'set!) `(*peer* ,name perm) `(content ,(latest)))
           ((self '~field) 'stage stage)))
