       ...
       (define (~signature-verify self chain public-key)
         ...
         (define* (~fetch self path slice?)
           (let* ((config ((self '~field) 'config))
                  ...
                  (window ((config 'get) '(public window)))
                  (current (- ((perm 'size)) 1)))
             (let* ((chain (if (< ((perm 'index) (car path)) (- current window)) perm ((self '~field) 'temp)))
                    (chain ((chain 'previous) (car path))))
               (if (eq? (caadr path) '*peer*)
                   (let* ((pref (reverse (list-tail (reverse path) 2)))
                          (name (cadadr pref))
                          (head ((standard 'load) (cadr ((standard 'deep-get) chain pref))))
                          (suff (cons (- ((head 'size)) 1) (list-tail path 3)))
                          (serialized ((eval ((config 'get) `(private peer ,name resolve))) suff))
                          (deserialized ((standard 'deserialize) serialized))
                     ...
                     (print 'hereer)
                     (if slice? ((standard 'deep-slice!) head path)) head)
                   (begin
                     (if (not slice?) chain ((standard 'deep-slice!) chain path)) chain)))))))
