    ...
    '(define-class (ledger)
       ...
       (define (~signature-sign! self chain)
         (let* ((config ((self '~field) 'config))
           ...
           ((standard 'deep-call!) chain '(-1)
            (lambda (record)
              ((record 'set!) '(*crypto* public-key) public-key)
              ((record 'set!) '(*crypto* signature) (crypto-sign secret-key (sync-digest (chain))))))))
       ...
       (define (~signature-verify self chain public-key)
         (let* ((config ((self '~field) 'config))
           ...
           ((standard 'deep-call!) chain '(-1)
            (lambda (record)
              (let ((included-key ((record 'get) '(*crypto* public-key)))
                    (signature ((record 'get) '(*crypto* signature))))
