(macro (path)
  (define src
    '(define-class (ledger)
       ...
       (define (~config-get self config path)
         (let loop ((config config) (path path))
           (if (null? path) config
               (let ((match (assoc (car path) config)))
                 (if (not match) #f
                     (loop (cadr match) (cdr path)))))))

       (define (~config-set self config path value)
         (let loop ((config config) (path path))
           (if (null? path) value
               (let loop-2 ((config config))
                 (cond ((null? config) (cons (car path) (loop '() (cdr path))))
                       ((eq? (caar config) (car path)) (cons (cons (car path) (loop (cdar config) (cdr path))) (cdr config)))
       ...
       (define (~signature-sign! self chain)
         (let* ((config ((self '~field) 'config))
           ...
           ((standard 'deep-set!) chain '(-1 (*crypto* public-key)) '(content #u()))
           ((standard 'deep-set!) chain '(-1 (*crypto* signature)) '(content #u()))
           ((standard 'deep-call!) chain '(-1)
            (lambda (record)
              ((record 'set!) '(*crypto* public-key) `(content ,public-key))
              ((record 'set!) '(*crypto* signature) `(content ,(crypto-sign secret-key (sync-digest (chain)))))))))

       (define (~signature-verify self chain public-key)
         (let* ((config ((self '~field) 'config))
                (standard ((self '~field) 'standard))
                (chain-copy ((standard 'load) (chain))))
           ((standard 'deep-set!) chain-copy '(-1 (*crypto* public-key)) '(content #u()))
           ((standard 'deep-set!) chain-copy '(-1 (*crypto* signature)) '(content #u()))
           ((standard 'deep-call!) chain '(-1)
            (lambda (record)
              (let ((included-key (cadr ((record 'get) '(*crypto* public-key))))
                    (signature (cadr ((record 'get) '(*crypto* signature)))))
         ...
         (define (*init* self standard config)
           (let ((record-class ((config 'get) '(private record-class)))
                 (chain-class ((config 'get) '(private chain-class))))
             ((self '~field) 'standard standard)
             ((self '~field) 'config config)
             ((self '~field) 'stage ((standard 'make) record-class))
             ((self '~field) 'temp ((standard 'make) chain-class))
             ((self '~field) 'perm ((standard 'make) chain-class))))

         (define* (config-private self (public? #t))
         ((self '~config-get) (byte-vector->expression (self ((self '~addr-config)))) '(private)))

       (define* (config-public self (public? #t))
         ((self '~config-get) (byte-vector->expression (self ((self '~addr-config)))) '(public)))
       ...
       (define (unpin! self path)
         ((self '~path-check) path #t)
         (let ((chain ((standard 'load) (self ((self '~addr-perm))))))
           ((standard 'deep-prune!) path)))
               ...
               (let ((chain ((eval (byte-vector->expression (sync-car node))) node)))
                 (if (> ((chain 'size)) 0)
                     (let* ((node ((chain 'get) -1))
                            (record ((eval (byte-vector->expression (sync-car node))) node)))
                       ((record 'get) '(*crypto* public-key))
                       ((record 'get) '(*crypto* signature))
                       ((chain 'digest) ,index))))))))

       (define (replicate self path)
         ((self '~path-check) path #t)
         (let ((obj ((self '~fetch) path #t)))
           ((standard 'serialize) chain (lambda (chain) ((standard 'deep-get) obj path)))))
         ...
         (let* ((config ((self '~field) 'config))
           ...
           (if (eq? ((config 'get) `(private peer ,name)) '()) #f
               (let* ((value ((standard 'deep-get) perm `(-1 (*peer* ,name chain))))
                      (last (if (eq? (car value) 'content) ((standard 'load) (cadr value)) #f))
       ...
       (define* (~fetch self path slice?)
         (let* ((config ((self '~field) 'config))
                (standard ((self '~field) 'standard))
                (perm ((self '~field) 'perm))
                (window ((config 'get) '(public window)))
                (current (- ((perm 'size)) 1)))
           (let ((obj (if (< ((perm 'index) (car path)) (- current window)) perm ((self '~field) 'temp))))
             (if (eq? (caadr path) '*peer*)
                 (let* ((pref (reverse (list-tail (reverse path) 2)))
                        (head ((standard 'load) ((standard 'deep-get) perm pref)))
                        (branch (eval ((self '~config-get) config `(private peer ,name resolve))))
                        (serialized (branch (list-tail 2)))
                        (deserialized ((standard 'deserialize) serialized))
                        (body ((standard 'load) deserialized)))
                   (if (not (equal? (sync-digest body) (head)))
                       (error 'digest-error "Remote chain does not match local chain head")
                       ((standard 'deep-call!) obj pref
                        (lambda (head)
                          (let ((standard 'deep-merge!) head body)))))
                   (if slice? ((standard 'deep-slice!) obj path)) obj)
                 (begin (if (not slice?) obj ((standard 'deep-slice!) obj path)) obj)))))))
