                            ...
                            (*call* ,(pass journal-name)
                                    (lambda (root)
                                      (let* ((std-node (cadr ((root 'get) '(control library standard))))
                                        ...
                                        ((root 'set!) '(control test ,obj-name) `(object ,(obj)))))) #t))))
         (query (lambda (args)
                  `(,(car args)
                    (*call* ,(pass (car args))
                            (lambda (root)
                              (let* ((std-node (cadr ((root 'get) '(control library standard))))
                                     ...
                                     (*result* ,(caddr args)))
                                ,@(map (lambda (x)
                                         `((root 'set!) '(control test ,x) `(object ,(,x))))
                                       (cadr args))
                                *result*)))
                    ,@(cdddr args)))))
    (run-test
     (append
      ...
      (map query
           `((journal (record-1) ((record-1 'set!) '(a b) 2) #t)
             ...
             (journal (record-1) ((record-1 'set!) '(a c* d) 4) #t)
             (journal (record-1) ((record-1 'get) '(a c d)) '(expression 4))
             ...
             (journal (record-1) ((record-1 'get) '(a b)) '(expression 2))
             (journal (record-1) ((record-1 'get) '(a c d)) '(expression 4))
             (journal (record-1) ((record-1 'get) '(a c* d)) '(expression 4))
             ...
             (journal (record-1) ((record-1 'get) '(b n c d)) '(expression 1))
             ...
             (journal (record-1) ((record-1 'equivalent?) '(b) '(b*)) #t)
             (journal (record-1) ((record-1 'get) '(b* d d)) '(expression 2))
             ))
      ))))
